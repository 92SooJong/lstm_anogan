...
class DCGAN(object):
    def __init__(self, img_rows=28, img_cols=28, channel=1, x_train=[], x_test=[]):
        ...
        self.x_train = np.array(self.x_train).reshape(-1, 14, 1).astype(np.float32)
        self.x_test = x_test
    ...
    def discriminator_model(self):
        ...
        self.D.add(Bidirectional(LSTM(12,return_sequences=True),input_shape=(14,1)))
        self.D.add(Bidirectional(LSTM(6,return_sequences=True)))
        self.D.add(Bidirectional(LSTM(3,return_sequences=True)))
        self.D.add(Bidirectional(LSTM(2,return_sequences=True)))
        self.D.add(Dense(1))
        self.D.add(Flatten())
        self.D.add(Dense(1))
    ...
    def generator_model(self):
        ...
        self.G.add(Bidirectional(LSTM(12,return_sequences=True),input_shape=(14,1)))
        self.G.add(Bidirectional(LSTM(6,return_sequences=True)))
        self.G.add(Bidirectional(LSTM(3,return_sequences=True)))
        self.G.add(Bidirectional(LSTM(2,return_sequences=True)))
        self.G.add(Bidirectional(LSTM(1,return_sequences=True)))
    ...
    def train(
        ...
        for e in range(epoch):
            for i in range(int(len(self.x_train)/batch_size)):
                total_step +=1
                noise = np.random.uniform(0, 1, size=[batch_size, 14,1])
                ...
                if (total_step) % predict_interval == 0:
                    ...
                    train_data, columns = model_predict.get_test_datas(path)

                    # path = '../SMUF_TRUE.csv'
                    # true_data, columns = model_predict.get_test_datas(path)

                    path = './sales_sample_false.csv'
                    false_data, columns = model_predict.get_test_datas(path)

                    intermidiate_model = self.get_feature_extractor()
                    data_num = 5

                    train_sum_score, train_score_list = model_predict.score(train_data, intermidiate_model, model, columns)
                    print("TRAIN AVG_Score : {}\n".format(train_sum_score / data_num))
                    false_sum_score, false_score_list = model_predict.score(false_data, intermidiate_model, model, columns)
