...
def get_test_datas(path):
    ...
    for c in columns:
        ...
        # date column ( 날짜형 )
        if c in date_list:
            temp = date_processing(c, new_test_data,df_index)
            temp.insert(0, min_date_value)
            temp.append(max_date_value)
            # the others (그외 데이터)
        else:
            # unique column ( 유니크 데이터)
...
def score(test_datas, intermidiate_model, model,columns):
    ...
    for test_img in test_datas:
        ...
        d_x = intermidiate_model.predict(test_img.reshape(1, 74, 1))

        loss = model.fit(z, [test_img.reshape(1, 74, 1), d_x], epochs=500, verbose=0)
        similar_data, _ = model.predict(z)
        
        # 가장 큰 값을 가지는 열 index 
        sub_image = np.subtract(similar_data,test_img.reshape(1,74,1))
